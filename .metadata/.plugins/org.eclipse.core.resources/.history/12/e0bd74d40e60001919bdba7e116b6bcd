...
<body>
                            ...
                            <table class="table table-striped table-bordered table-hover" id="dataTables-employee">
								...
								<tbody id = "dataTables-employee-body">
									...
									<tr class="odd gradeX">
										...
										<button id = "dataTables-payslip-td" class="btn btn-sm btn-primary">급여명세서</button> 
										<button id = "dataTables-empdel-td" class="btn btn-sm btn-warning">퇴직처리</button></td>
										<td hidden="hidden" id = "scontdate" >${record.SCONTDATE.substring(0,10)}</td>
										<td hidden="hidden" id = "econtdate">${record.ECONTDATE.substring(0,10)}</td>
										<td hidden="hidden" id = "sprbdate">${record.SPRBDATE.substring(0,10)}</td>
										<td hidden="hidden" id = "eprbdate">${record.EPRBDATE.substring(0,10)}</td>
										<td hidden="hidden" id = "prbrate">${record.PRBRATE}</td>
									</tr>
									</c:forEach>
									</c:when>
									<c:otherwise>
	  ...
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
			<form id = "frmCntrctModal" action="/pay/prtCntrct.pay" method="post">
				<input type="hidden" name="COMCODE" value="${record.COMCODE}">
				<div class="form-group">
					<label>사원번호:</label>
					<input class="form-control" name="EMPNO" type="text" id = "cntrctempnoModal" value="${record.EMPNO}" readonly="readonly">
				</div>
				<div class="form-group">
					<label>사원명:</label>
					<input class="form-control" name="EMPNAME" type="text" id = "cntrctempnameModal" value="${record.EMPNAME}" readonly="readonly">
				</div>
				<div class="form-group">
					<label>계약시작일:</label>
	...
	<!-- 급여명세서 Modal -->
	<div class="modal" tabindex="-1" role="dialog" id="paySlipModal">
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <h4 class="modal-title">급여명세서 요청</h4>
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
	          <span aria-hidden="true">&times;</span>
	        </button>
	      </div>
	      <div class="modal-body">
			<form id = "frmPaySlipModal" action="/pay/prnPaySlip.pay" method="post">
				<input type="hidden" name="COMCODE" value="${record.COMCODE}">
				<div class="form-group">
					<label>사원번호:</label>
					<input class="form-control" name="EMPNO" type="text" id = "payslipempnoModal" value="${record.EMPNO}" readonly="readonly">
				</div>
				<div class="form-group">
					<label>사원명:</label>
					<input class="form-control" name="EMPNAME" type="text" id = "payslipempnameModal" value="${record.EMPNAME}" readonly="readonly">
				</div>
				<div class="form-group">
					<label>년월</label>
					<input class="form-control" name="YYYYMM" type="number" id = "payslipyyyymmModal">
				</div>
			</form>
	      </div>
	      <div class="modal-footer">
	        <button type="button" class="btn btn-sm btn-secondary" data-dismiss="modal">닫기</button>
	        <button id = "btnPaySlipModal" type="button" class="btn btn-sm btn-primary">요청</button>
	      </div>
	    </div>
	  </div>
	</div>
	...
	<div class="modal" tabindex="-1" role="dialog" id="delEmpModal">
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
			<form id = "frmDelEmpModal" action="/pay/delEmployee.pay" method="post">
				<input type="hidden" name="COMCODE" value="${record.COMCODE}">
				<div class="form-group">
					<label>사원번호:</label>
					<input class="form-control" name="EMPNO" type="text" id = "empnoModal" value="${record.EMPNO}" readonly="readonly">
				</div>
				<div class="form-group">
					<label>사원명:</label>
					<input class="form-control" name="EMPNAME" type="text" id = "empnameModal" value="${record.EMPNAME}" readonly="readonly">
				</div>
				<div class="form-group">
					<label>퇴직일자:</label>
					<input class="form-control" name="OUTDATE" type="Date" id = "outdateModal" value="${record.OUTDATE}">
				</div>
				<div class="form-group">
					<label>퇴직사유:</label>
					<input class="form-control" name="OUTREASON" type="text" id = "outReasonModal" value="${item.OUTREASON}">
				</div>
			</form>
	      </div>
	      <div class="modal-footer">
    ...
    <script>
     $(document).ready(function() {
         ...
         $('#dataTables-employee-body').on('click','#dataTables-cntrct-td',function(e){
         	var tag = $(this).parents();
    		var empno = tag.siblings('#empno').text();
    		var empname = tag.siblings('#empname').text();
    		var scontdate = tag.siblings('#scontdate').text();
    		var econtdate = tag.siblings('#econtdate').text();
    		var sprbdate = tag.siblings('#sprbdate').text();
    		var eprbdate = tag.siblings('#eprbdate').text();
    		var prbrate = tag.siblings('#prbrate').text();
    		$('#cntrctempnoModal').val(empno);
    		$('#cntrctempnameModal').val(empname);
    		$('#cntrctscontdateModal').val(scontdate);
    		$('#cntrctecontdateModal').val(econtdate);
    		$('#cntrctsprbdateModal').val(sprbdate);
    		$('#cntrcteprbdateModal').val(eprbdate);
    		$('#cntrctprbrateModal').val(prbrate);
    		$('#cntrctModal').modal('show');
         });

         $('#dataTables-employee-body').on('click','#dataTables-payslip-td',function(e){
         	var tag = $(this).parents();
   			var empno = tag.siblings('#empno').text();
   			var empname = tag.siblings('#empname').text();
   			$('#payslipempnoModal').val(empno);
   			$('#payslipempnameModal').val(empname);
   			
   			$('#paySlipModal').modal('show');
         });
         
        $('#dataTables-employee-body').on('click','#dataTables-empdel-td',function(e){
        	var tag = $(this).parents();
   			var empno = tag.siblings('#empno').text();
   			var empname = tag.siblings('#empname').text();
   			var outDate = tag.siblings('#outDate').text();
   			var outReason = tag.siblings('#outReason').text();
   			$('#empnoModal').val(empno);
   			$('#empnameModal').val(empname);
   			$('#outDateMpdal').val(outDate);
    		$('#outReaonModal').val(outReason);
   		...
   		$('#btnPaySlipModal').click(function(){
   			$('#frmPaySlipModal').submit();
   		});
